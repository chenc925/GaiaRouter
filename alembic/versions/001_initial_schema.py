...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(length=20), server_default='active', nullable=True, comment='状态：active/inactive'),
        sa.Column('monthly_requests_limit', sa.Integer(), nullable=True, comment='月度请求次数限制'),
        sa.Column('monthly_tokens_limit', sa.Integer(), nullable=True, comment='月度Token限制'),
        sa.Column('monthly_cost_limit', sa.Numeric(precision=10, scale=2), nullable=True, comment='月度费用限制'),
    ...
    op.create_table(
        ...
        sa.Column('key_hash', sa.String(length=255), nullable=False, comment='API Key哈希值（加密存储）'),
        sa.Column('permissions', sa.JSON(), nullable=True, comment='权限列表'),
        sa.Column('status', sa.String(length=20), server_default='active', nullable=True, comment='状态：active/inactive/expired'),
    ...
    op.create_table(
        ...
        sa.Column('prompt_tokens', sa.Integer(), server_default='0', nullable=True, comment='输入Token数'),
        sa.Column('completion_tokens', sa.Integer(), server_default='0', nullable=True, comment='输出Token数'),
        sa.Column('total_tokens', sa.Integer(), server_default='0', nullable=True, comment='总Token数'),
    ...
    op.create_index(op.f('ix_request_stats_timestamp'), 'request_stats', ['timestamp'], unique=False)
    op.create_index(op.f('ix_request_stats_api_key_id'), 'request_stats', ['api_key_id'], unique=False)
    op.create_index(op.f('ix_request_stats_organization_id'), 'request_stats', ['organization_id'], unique=False)
