...
5. **API Key 管理**
   ...
   - 支持 API Key 的创建、查询、删除
   - 支持 API Key 的启用/禁用（通过状态字段）
   - 支持 API Key 的权限管理（固定为 read、write）
   - 提供 API Key 的后台管理界面和 API 接口
   - **业务规则**：
     - 一个组织只能创建一个活跃的 API Key
     - API Key 创建时自动生成名称和描述
     - 不支持更新 API Key（需删除后重建）
   - **安全策略**：
     - API Key 仅在创建时返回一次（完整明文）
     - 查询接口（列表/详情）不返回完整 key（返回 null）
     - 前端隐藏显示（sk-****...****格式）
     - 多层防护：后端不返回 + 前端隐藏
...
6. ✅ 支持模型列表查询
7. ✅ 提供完整的 API 文档
8. ✅ 支持 API Key 的完整生命周期管理（创建、查询、删除）
