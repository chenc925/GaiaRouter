...
**描述**：提供基于 Vue 3 + Arco Design Vue 的后台管理界面，用于管理组织、API Keys 和查看统计数据。
...
- **页面结构**：
  ...
  - **路由设计**：
    ...
    - `/organizations/:id/edit` - 编辑组织
    - `/api-keys` - API Key 列表
    - `/api-keys/:id` - API Key 详情
    - `/api-keys/:id/edit` - 编辑 API Key
    - `/stats` - 统计概览
    - `/stats/organizations/:id` - 组织统计
    - `/stats/api-keys/:id` - API Key 统计
    - `/settings` - 系统设置（可选）
