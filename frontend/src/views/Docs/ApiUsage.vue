  ...
  <div class="api-usage">
    ...
    <a-card class="section-card" title="2. 获取可用模型列表 (GET /v1/models)" :bordered="false">
      <a-typography>
        <a-typography-paragraph>
          终端应用在发起聊天前，应先调用
          <code>/v1/models</code> 获取可用模型列表，只能调用已启用的模型。
        </a-typography-paragraph>
  ...
  -d '{
    ...
    "stream": true
        ...
        <a-typography-paragraph>
          <ul>
            <li>
              <strong>model</strong>：模型标识符，例如 <code>openai/gpt-4</code> 或
              <code>openrouter/xxx</code>。
            </li>
            <li>
              <strong>messages</strong>：对话消息数组，角色支持 <code>system</code> /
            ...
            <li>
              <strong>stream</strong>：是否开启流式输出，终端可按行解析 <code>data:</code> 前缀。
            </li>
          </ul>
        </a-typography-paragraph>
      </a-typography>
